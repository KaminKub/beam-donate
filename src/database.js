...
async function migrateDB() {
  ...
  try {
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS streamers (
        ...
        goal_bg_settings TEXT DEFAULT NULL,
         ...
         payment_eligibility_accepted_at TEXT DEFAULT NULL,
        ...
        truemoney_webhook_verified_at TEXT,
        ...
        tts_quota_guard_enabled INTEGER DEFAULT 1
      )
    `);
    await db.execute(`
    ...
    const requiredCols = [
      ...
      { name: 'payment_eligibility_version', type: 'TEXT DEFAULT NULL' },
      { name: 'payment_eligibility_accepted_at', type: 'TEXT DEFAULT NULL' },
      ...
      { name: 'truemoney_webhook_verified_at', type: 'TEXT' },
  ...
  if (!db) throw new Error('Database not initialized');
   ...
   for (const field of PAYMENT_ENCRYPT_FIELDS) {
     if (data[field]?.length && data[field].includes('*')) {
       ...
       delete data[field];   // strip plaintext — else it survives into finalData and leaks via response/broadcast
     }
   }
        ...
        sql: `UPDATE streamers SET
              ...
              social_discord = COALESCE(?, streamers.social_discord),
               ...
               goal_pointer_content = COALESCE(?, streamers.goal_pointer_content),
                ...
                payment_eligibility_version = COALESCE(?, streamers.payment_eligibility_version),
                payment_eligibility_accepted_at = COALESCE(?, streamers.payment_eligibility_accepted_at),
               ...
               truemoney_webhook_verified_at = COALESCE(?, streamers.truemoney_webhook_verified_at),
       ...
       args: [
         ...
         finalData.discord_webhook_url || null,
          ...
          finalData.goal_pointer_content !== undefined ? finalData.goal_pointer_content : null,
           ...
           finalData.payment_eligibility_version !== undefined ? finalData.payment_eligibility_version : null,
           finalData.payment_eligibility_accepted_at !== undefined ? finalData.payment_eligibility_accepted_at : null,
          ...
          finalData.truemoney_webhook_verified_at !== undefined ? finalData.truemoney_webhook_verified_at : null,
     ...
     const _insertResult = await db.execute({
       sql: `INSERT INTO streamers (twitch_id, streamlabs_id, streamlabs_username, username, discord_webhook_url, overlay_token, is_active, 
             ...
             customImageMode, customImageValue, customSoundUrl, alert_sound_url, page_title, page_subtitle, thank_you_header, thank_you_subtitle,
              ...
              truemoney_webhook_secret_encrypted, truemoney_webhook_enabled, truemoney_webhook_kyc_confirmed, truemoney_webhook_expiry, truemoney_webhook_methods, truemoney_promptpay_id_encrypted, truemoney_webhook_verified_at, badges, badge_display, badge_display_top, badge_optout, leaderboard_settings, recentdonate_settings, goal_text_settings, tier_donate_settings, sound_library, goal_bar_width_auto,
       ...
       args: [
        ...
        finalData.goal_pointer_content || 'both',
         ...
         finalData.payment_eligibility_version || null,
         finalData.payment_eligibility_accepted_at || null,
        ...
        finalData.truemoney_webhook_verified_at || null,
        finalData.badges || '{}',
        finalData.badge_display !== undefined ? finalData.badge_display : null,
        finalData.badge_display_top !== undefined ? finalData.badge_display_top : null,
        finalData.badge_optout !== undefined ? finalData.badge_optout : null,
...
/**
 * Record payment-connector eligibility self-attestation. Reuses legal_acceptances
 * with a namespaced version string so it can't collide with CURRENT_LEGAL_VERSION rows.
 */
async function recordPaymentEligibilityAcceptance(streamerId, eligibilityVersion, acceptedAt) {
  await ensureConnected();
  if (isFallback || !db) throw new Error('Database unavailable');

  const id = Number(streamerId);
  if (!Number.isInteger(id) || id <= 0) throw new Error('Invalid streamer id');
  if (typeof eligibilityVersion !== 'string' || !eligibilityVersion.trim()) throw new Error('Invalid eligibility version');
  if (typeof acceptedAt !== 'string' || Number.isNaN(Date.parse(acceptedAt))) throw new Error('Invalid acceptance timestamp');
  ...
  const namespacedVersion = `PAYMENT_ELIGIBILITY_${eligibilityVersion}`;

  const statements = [
    {
      sql: `INSERT OR IGNORE INTO legal_acceptances (streamer_id, legal_version, accepted_at)
            VALUES (?, ?, ?)`,
      args: [id, namespacedVersion, acceptedAt]
    },
    {
      sql: 'UPDATE streamers SET payment_eligibility_version = ?, payment_eligibility_accepted_at = ? WHERE id = ?',
      args: [eligibilityVersion, acceptedAt, id]
    }
  ];
  ...
  const acceptanceResult = await db.execute({
    sql: `SELECT accepted_at
          FROM legal_acceptances
          WHERE streamer_id = ? AND legal_version = ?`,
    args: [id, namespacedVersion]
  });
  const acceptance = acceptanceResult.rows[0];
  if (!acceptance) throw new Error('Payment eligibility acceptance proof was not persisted');

  return {
    acceptedVersion: eligibilityVersion,
    acceptedAt: acceptance.accepted_at
  };
}
...
module.exports = {
  ...
  recordPaymentEligibilityAcceptance,
};
